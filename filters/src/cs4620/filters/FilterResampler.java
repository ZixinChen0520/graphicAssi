...
public class FilterResampler implements ResampleEngine {
    ...
    public void resample(SimpleImage src, SimpleImage dst, double left, double bottom, double right, double top) {
        ...
        float tempY1 = 0F;
        float tempY2 = 0F;
        float stepX = (float)(right - left)/(float)dstWidth;
        float stepY = (float)(top - bottom)/(float)dstHeight;
        ...
        float radiusY = stepY <= 1 ? origRadius : stepY * origRadius;
            ...
            for (jDst = 0; jDst < dstHeight; jDst++){
                ...
                for (mflt = 0; mflt < filterSize; mflt++){
                    ...
                    for (nflt = 0; nflt < filterSize; nflt++){
                        ...
                        xPos = (int)(Math.ceil(iDst * scaleX - radiusX + nflt));
                        yPos = (int)(Math.ceil(jDst * scaleY - radiusY + mflt));
                        // Boundary Conditions
                        ...
                        tempX0 += filter.evaluate((float) ((radiusX - nflt)/scaleX)) * (float)(src.getPixel(xPos, yPos, 0) & 0xff);
                        tempX1 += filter.evaluate((float) ((radiusX - nflt)/scaleX)) * (float)(src.getPixel(xPos, yPos, 1) & 0xff);
                        tempX2 += filter.evaluate((float) ((radiusX - nflt)/scaleX)) * (float)(src.getPixel(xPos, yPos, 2) & 0xff);
                    }
                ...
                for  (kflt = 0; kflt < filterSize; kflt++){
                    tempY0 += filter.evaluate(radiusY - kflt) * filterResultTemp0[kflt];
                    tempY1 += filter.evaluate(radiusY - kflt) * filterResultTemp1[kflt];
                    tempY2 += filter.evaluate(radiusY - kflt) * filterResultTemp2[kflt];
                }
                //System.out.println(tempY0);
                if (tempY0 < 0) {tempY0 = 0;}
                else if (tempY0 > 255) {tempY0 = 255;}

                if (tempY1 < 0) {tempY1 = 0;}
                else if (tempY1 > 255) {tempY1 = 255;}

                if (tempY2 < 0) {tempY2 = 0;}
                else if (tempY2 > 255) {tempY2 = 255;}
